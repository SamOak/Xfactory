...
public class TransformFileService implements ITransformFileService {
	...
	public Collection<TransformFile> getFiles(String area, String filter) {
		...
		final ArrayList<TransformFile> coll = new ArrayList<TransformFile>();

		// Search all file in this area
		final File[] slotList = new File(basePath + "/" + area).listFiles(File::isDirectory);
		if (slotList != null) {
			for (final File slot : slotList) {

				final Pattern p = Pattern.compile(filter);
				final File[] fileList = new File(basePath + "/" + area + "/" + slot.getName())
