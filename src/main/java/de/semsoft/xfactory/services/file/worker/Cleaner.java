...
public class Cleaner {
	...
	public void cleanINArea() {
		// LOG.info("Clean finished files .... ");

		// move done files
		final Collection<TransformFile> doneFileList = transformFileService.getFiles(inAreaName, fileFilterRegexDone);
		for (final TransformFile doneFile : doneFileList) {

			LOG.info("Found (" + doneFile.getFileName() + ") as done");
			try {
				final TransformFile orgFile = doneFile.getOriginalFile();

				Files.createDirectories(Paths.get(basePath + "/" + doneAreaName + "/" + orgFile.getSlot()));
				Files.move(orgFile.getPath(), Paths
						.get(basePath + "/" + doneAreaName + "/" + orgFile.getSlot() + "/" + orgFile.getFileName()));
				doneFile.delete();
		...
		if (jobMaxDelay > 0) {
			final Collection<TransformFile> jobFileList = transformFileService.getFiles(inAreaName,
					fileFilterRegexDone);
			for (final TransformFile jobFile : jobFileList) {
				if (jobFile.getAgeMilliSecond() > jobMaxDelay) {
					LOG.info("Jobfile (" + jobFile.getFullFileName() + ") is (" + jobFile.getAgeMilliSecond()
							+ ") milliseconds old. Maxvalue is (" + jobMaxDelay
							+ "). Probably job is dead. We delete the jobfile to rerun");
					jobFile.delete();
				}
			}
		}
